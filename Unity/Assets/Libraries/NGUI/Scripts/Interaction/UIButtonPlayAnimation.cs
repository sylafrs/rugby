...
public class UIButtonPlayAnimation : MonoBehaviour
{
	...
	/// <summary>
	/// Which event will AnimationOrTween.Trigger the animation.
	/// </summary>
	...
	/// <summary>
	/// Event receiver to AnimationOrTween.Trigger the callback on when the animation finishes.
	/// </summary>
	...
	void OnHover (bool isOver)
	{
		if (enabled)
		{
			if ( trigger == AnimationOrTween.Trigger.OnHover ||
				(trigger == AnimationOrTween.Trigger.OnHoverTrue && isOver) ||
				(trigger == AnimationOrTween.Trigger.OnHoverFalse && !isOver))
			{
				Play(isOver);
			}
			mHighlighted = isOver;
	...
	void OnPress (bool isPressed)
	{
		if (enabled)
		{
			if ( trigger == AnimationOrTween.Trigger.OnPress ||
				(trigger == AnimationOrTween.Trigger.OnPressTrue && isPressed) ||
				(trigger == AnimationOrTween.Trigger.OnPressFalse && !isPressed))
			{
				Play(isPressed);
			}
		}
	...
	void OnClick ()
	{
		if (enabled && trigger == AnimationOrTween.Trigger.OnClick)
		{
			Play(true);
		}
	}
	...
	void OnDoubleClick ()
	{
		if (enabled && trigger == AnimationOrTween.Trigger.OnDoubleClick)
		{
			Play(true);
		}
	}
	...
	void OnSelect (bool isSelected)
	{
		if (enabled)
		{
			if (trigger == AnimationOrTween.Trigger.OnSelect ||
				(trigger == AnimationOrTween.Trigger.OnSelectTrue && isSelected) ||
				(trigger == AnimationOrTween.Trigger.OnSelectFalse && !isSelected))
			{
				Play(true);
			}
		}
	...
	void OnActivate (bool isActive)
	{
		if (enabled)
		{
			if (trigger == AnimationOrTween.Trigger.OnActivate ||
				(trigger == AnimationOrTween.Trigger.OnActivateTrue && isActive) ||
				(trigger == AnimationOrTween.Trigger.OnActivateFalse && !isActive))
			{
				Play(isActive);
			}
		}
