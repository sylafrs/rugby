...
public class DropManager
{
    ...
    public void DoRebond(float t)
    {
        switch (type)
        {
            case TYPEOFDROP.KICK: doRebondKick(t);
                break;
            case TYPEOFDROP.UPANDUNDER: //doRebondUpAndUnder(t);
                break;
            default: break;
        }
    }

    public void setupRebond()
    {
        directionRebond = new Vector3((float)Game.instance.rand.Next(3), Mathf.Sin(Mathf.Deg2Rad * 75f), (float)Game.instance.rand.Next(3));
        initPos = ball.transform.position;
        initPos.y = Ball.epsilonOnGround + 0.1f;
    }

    private void doRebondKick(float t)
    {

        Vector3 newPos = new Vector3(directionRebond.x * t + initPos.x,
             -0.5f * 9.81f * t * t + 8f * directionRebond.y * t + initPos.y,
             directionRebond.z * t + initPos.z);
    ...
    private void doKick(float t)
    {
        Vector3 newPos = new Vector3((ownerDirection.x * ball.multiplierDropKick.y + (angleX >= 0f ? Mathf.Cos(angleX) : -Mathf.Cos(angleX))) * t + initPos.x,
                                                acceleration * 9.81f * t * t + ball.multiplierDropKick.x * Mathf.Sin(Mathf.Deg2Rad * Game.instance.settings.GameStates.MainState.PlayingState.MainGameState.RunningState.BallFreeState.angleDropKick) * t + initPos.y,
