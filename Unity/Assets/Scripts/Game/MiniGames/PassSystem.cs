		...
		if (from && target && ball)
		{
			...
			*/
			...
			foreach (Unit u in target.Team)
			{
				if (u != target)
				{
					u.Order = Order.OrderOffensiveSide(target, new Vector3(ball.Game.settings.Global.Game.Vheight, 0, ball.Game.settings.Global.Game.Vwidth), target.Team.right, typePosition);
