...
public class scrumController : myMonoBehaviour {
    ...
    private float GetSmashResult()
    {
        ...
        if (Game.right.Player.XboxController.GetButtonDown(Game.settings.Inputs.rightSmashButton.xbox) || 
			Input.GetKeyDown(Game.settings.Inputs.rightSmashButton.keyboard(Game.right)))
        {
        ...
        if (Game.left.Player.XboxController.GetButtonDown( Game.settings.Inputs.leftSmashButton.xbox) ||
			Input.GetKeyDown(Game.settings.Inputs.leftSmashButton.keyboard(Game.left)))
        {
            smash -= Game.settings.Global.Game.SmashValue;
            this.SuperLoading = Mathf.Min(1, this.SuperLoading + Game.settings.Global.Game.FeedSuperPerSmash);
        }
        ...
        if (this.SuperLoading == 1)
        {
            ...
            if (Game.right.Player.XboxController.GetButtonDown(Game.settings.Inputs.rightSuperButton.xbox) 
				|| Input.GetKeyDown(Game.settings.Inputs.rightSuperButton.keyboard(Game.right)))
            {
                super += 1;
                used = true;
            }

            if (Game.left.Player.XboxController.GetButtonDown(Game.settings.Inputs.leftSuperButton.xbox) 
				|| Input.GetKeyDown(Game.settings.Inputs.leftSuperButton.keyboard(Game.left)))
            {
                super -= 1;
                used = true;
            }
